...
function construct_two_local_microcircuit_integrator(; dend_param=dendrites_param_sigmoid(0.12, -7.0, -0.482, 0.00964, 0.19624, 0.0), facilitation=false, adaptation=false, depression=false, td_to_vip=true, τ_integrator = 0.8)
    ...
    lc = construct_two_local_microcircuit_integrator(facilitation=true, depression=true, adaptation=true,  τ_integrator = τ_integrator)
...
function map_from_dict(dict::Dict{String,Float64})
    produce_or_load(datadir("sims", "script2"), dict, generate_oddball_task, prefix="oddball_task")
end
...
general_args = Dict(
	...
	"Tstim" => 0.5,
	"Tfin" => 120.0,
	"value_stim_f2" => collect(0.0:0.05:0.2),
    "τ_integrator" => collect(0.1:0.1:1.5)
)
