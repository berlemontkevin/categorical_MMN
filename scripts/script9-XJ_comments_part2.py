...
def run_parallel(list_param):
    ...
    name = 'plast_int'+str(list_param[6]) + '_plast_ndf'+ str(list_param[5])+'dev_id'+str(list_param[3])+'adapt_'+str(list_param[0])+'topdown_'+str(list_param[1])+'Tinter_'+str(list_param[2])+'delay_'+str(list_param[4])+'broadtodend_'+str(list_param[7])+'broadtondf_'+str(list_param[8])+'adaptationtc_'+str(list_param[9])+ 'tau_int_'+str(list_param[10])
    ...
    #             json.dump(save_dict, handle, cls = cl.NumpyEncoder)
        ...
        c.execute("""INSERT INTO table_MMN_effect (name_id 
          ...
          , idx_adaptation 
          , sum_idx_adaptation
           )""" """VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""", (name,list_param[0],list_param[1],list_param[2],list_param[10],list_param[3],list_param[4],list_param[5],list_param[6],list_param[7],list_param[8],saving_multi_control,saving_proba_control,saving_proba_dev,sum_control,sum_std,sum_dev,mmn, mmn_norm, sum_mmn, sum_mmn_norm, idx_adaptation, sum_idx_adaptation))
        con.commit()
        con.close()        
        lock.release()
    else:
